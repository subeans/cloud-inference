...
# fake dataset 
def make_dataset(model_name, batch_size, seq_length, dtype="float32"):
...
def inference(saved_model_dir,batch_size,max_seq_length,repeat=10,warm_up=3):
  ...
  #X_train, y_train, X_test, y_test , index_word = get_dataset()
    ...
    for i in range(repeat):
        ...
        # fake dataset 
        make_dataset = time.time()
        input_ids, attention_masks, valid_length = make_dataset(saved_model_dir, batch_size, max_seq_length)
        d_load_time = time.time()-make_dataset
...
if __name__ == "__main__":
    ...
    for batch_size in batch_list:
        ...
        if save and batch_size==1 : 
            input_ids, token_types, valid_length = make_dataset(model, batch_size, max_seq_length)
